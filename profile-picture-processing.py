...
while image_name != "end":
    with Image.open(image_name) as im:
        ...
        # left = Image.fromarray(np_data[0:y//2, 0:strip_size])
        # right = Image.fromarray(np_data[0:y//2, x-strip_size:x])
        background = Image.new('RGB', (size, size))

        # piece together parts of the image
        for i in range(0, 9):
            background.paste(left, (i*strip_size, 0))
            # background.paste(ImageOps.flip(left), (i*strip_size, y//2))
        for i in range(0, 13):
            background.paste(right, (3150+i*strip_size, 0))
            # background.paste(ImageOps.flip(right), (3150+i*strip_size, y//2))
